...
if(!isset($_POST['pw_reset']) && !isset($_POST['pw_reset_confirm'])) header("Location: /app/passwordreset?user_id={$_POST['user_id']}&key={$_POST['key']}&error=nopwset");

// If password and confirmation do not match, return error
if($_POST['pw_reset'] !== $_POST['pw_reset_confirm']) header("Location: /app/passwordreset?user_id={$_POST['user_id']}&key={$_POST['key']}&error=nomatchpw");
