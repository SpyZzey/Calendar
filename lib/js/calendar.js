...
function submitEvent(edit_type) {
    ...
    pushEventToServer(edit_type, title, eventId, description, event_color, all_day, start, end, type);
    resetAddEntryModal();
    ...
    $.ajax({
        ...
        success: function (data) {
            if(data === "not logged in") {
                ...
                const firstWeekdayOfMonth = getWeekdayOfDate(1, currentMonth, currentYear);
                fetchCalendarEventsByMonth(currentMonth, currentYear, days, firstWeekdayOfMonth);
            } else {
...
function calculateEventPosition(currentPos, time, firstWeekdayOfMonth) {
    ...
    if(time < monthStart) {
        ...
        let daysAfter = calendarRows * calendarCols - daysInCurrentMonth;
...
$('#button-event-delete').on('click', function () {
    console.log("delete " + $(this).parents('#modal-event').data('event_id'));
